...
Im vorherigen Abschnitt wurde bereits die Trajektorie der verschiedenen Odometriequellen untersucht. Mit dem rf2o-Verfahren wird die Positionsschätzung des roslam-Verfahrens untersucht. Es wird dabei die Trajektorie mit dem abgerundeten Rechteck mit mehrere Kreisbahnen verwendet, siehe \autoref{fig:Record_2018-02-08-12-33-53_trajectory}. Da auch die Entfernungsmessungen der uwbm einen Einfluss auf die Positionsschätzung besitz, werden neben den echten Entfernungmessungen auch Entfernungsmessungen von perfekten virtuellen uwbm verwendet.

In der Abbildung ... ist die tätschliche Trajektorie und die des Partikel Filters abgebildet. Die oben zwei Bilder verwenden die Entfernungsmessungen der echten uwbm, wobei die unteren zwei Bilder die Entfernungsmessung von virtuellen uwbm verwenden. Es ist eindeutigen zu erkennen das die Genauigkeit der Entfernungsmessung einen großen Einfluss auf die Positionsschätzung besitzt. Die oberen zwei Positionsschätzungen weisen starke Abweichungen von der tatsächlichen Trajektorie auf. Erstaunlich ist es auch, das das Abspielen des gleichen Datensatzes immer wieder zu einer anderen Trajektorie führt. Dieses Verhalten lässt sich nur bedingt dadurch erklären, das beim Resampling zufällige Partikel erzeugt werden.

Die Abweichung bei den virtuellen uwbm fällt deutlich geringer aus, wie man in den Abbildung (c) und (d) sehen kann. Daher liegt die Vermutung sehr nahe, das die Entfernungsmessung der uwbm nicht ausreichtend genau ist .
...
\begin{figure}
	...
	\begin{subfigure}{0.49\linewidth}
		...
		\caption{Trajektorie mit echten UWB-Modulen.}
