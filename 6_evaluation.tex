...
Der \Gls{anchor} hat nach ca. \SI{17}{\hour} seinen Dienst eingestellt, wenig später folgte ihm der \Gls{tag}. Deutlich höhere Batterielaufzeiten können dadurch erzielt werden, dass die Senderate reduziert wird und die Stromsparfunktionen sowohl des DWM1000 als auch des ATmega328/P genutzt werden.
...
Anzumerken ist, dass das \gls{roslam} Verfahren von der Annahme ausgeht das die Position der \glspl{uwbm} sich über die Zeit nicht ändert. Während der Auswertung der Daten konnte jedoch mehrfach beobachtet werden, dass sich die Positionen der \glspl{uwbm} über die Zeit änderten. Somit ist nicht ausgeschlossen, dass die Positionsänderungen der \glspl{uwbm} verfolgt werden kann, sobald die \gls{wdf} durch das \gls{ekf} Verfahren repräsentiert wird.
...
Einige Entfernungen kommen pro Spalte doppelt bzw. dreifach hintereinander vor. Diese entstehen dadurch, das das \gls{mrpt} \gls{rosm} in großen Zeiträumen keine neuen Positionsschätzungen für diese \glspl{uwbm} bereitgestellt hatte. Aus diesem Grund wurde die letzte bekannte Entfernung angegeben. Die Ursache für dieses Phänomen konnte bis zum Schluss nicht geklärt werden.
