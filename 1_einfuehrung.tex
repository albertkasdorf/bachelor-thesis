...
Außerhalb von geschlossenen Räumlichkeiten (engl. Outdoor) erfolgt die Lokalisierung in der Regel mittels GPS, unter der Voraussetzung das eine ungehinderte Verbindung (engl. \glsreset{slam}\gls{los}) zu den GPS-Satelliten möglich ist. Die Lokalisierung ist in diesem Fall sehr einfach, da die GPS Koordinaten eindeutig sind und das Kartenmaterial bereits im gleichen Koordinatensystem vorliegt.
...
Im Nachgang wird dann der \Gls{roslam}-Al\-go\-rith\-mus aus dem \Gls{mrpt}-Framework mit den Aufzeichnungen erprobt. Untersucht werden dabei die Genauigkeit der Roboter Lokalisierung und das Erstellen einer Karte von den zuvor unbekannten \Gls{anchor}-Positionen.
